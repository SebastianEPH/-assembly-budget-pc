...
export const Disk = ({data, others, modalHandleClose}) =>{
    const {modal,dataBrand, diskSize, diskType, reloadForDB, dataStore} = others
    ...
    let  {name, id,  index, store, brand, link ,item_active, dol, sol, size, type } = form
                    ...
                    <Row className="accordion-body">
                        <Col md={11}>
                            <Row className={"pe-2 ps-2 pt-2"}>
                                ...
                                <InputSelect col={3} name={"size"} title={"Memory Size"} update={onChange} select={size}  data={diskSize}/>
