...
export const MemoryRam = ({data, others, modalHandleClose}) =>{
    const {modal, dataBrand, reloadForDB, dataStore, dataSize, dataFreq, dataType} = others
    ...
    let  {name, id, index, store, brand,size, freq, type,  link ,item_active, dol, sol} = form
                ...
                {!modal &&
                    <h2 className="accordion-header">
                        <button className="accordion-button" type="button" data-bs-toggle="collapse" data-bs-target={`#panelsStayOpen-collapseOne_${nameDiv}_${id}`} aria-expanded="false" >
                            <b className={"me-3"}>Memory RAM {index !== 0 && `${(index+1)}`}: </b> {name} <TextActive data={{state:item_active, center:false}}/>
                        </button>
                    </h2>
                }
                    ...
                    <Row className="accordion-body">
                        <Col md={11}>
                            <Row className={"pe-2 ps-2 pt-2"}>
                                ...
                                <InputSelect col={2} name={"size"} title={"Size"} update={onChange} select={size}  data={dataSize}/>
                                <InputSelect col={2} name={"type"} title={"Type"} update={onChange} select={type}  data={dataType}/>
                                <InputSelect col={2} name={"freq"} title={"Frequency"} update={onChange} select={freq}  data={dataFreq} idCondition={type} />
