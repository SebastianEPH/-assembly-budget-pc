...
export const Processor = ({data, others, modalHandleClose}) =>{
    const {modal, processorType, reloadForDB, dataStore} = others
                ...
                {!modal &&
                    <h2 className="accordion-header">
                        <button className="accordion-button" type="button" data-bs-toggle="collapse" data-bs-target={`#panelsStayOpen-collapseOne_${nameDiv}_${id}`} aria-expanded="false" >
                            <b className={"me-3"}>Processor {index !== 0 && `${(index+1)}`}: </b> {name} <TextActive data={{state:item_active, center:false}}/>
                        </button>
                    </h2>
                }
                    ...
                    <Row className="accordion-body">
                        <Col md={11}>
                            <Row className={"pe-2 ps-2 pt-2"}>
                                <InputsCustom data={{col:12 ,name:"name", placeHolder:"inserta name", value:name, onChange, updateHook:""}}/>
                                <InputSelect col={3} name={"brand"} title={"brand"} update={onChange} select={brand}  data={processorType}/>
