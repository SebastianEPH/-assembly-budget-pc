...
export const Motherboard = ({data, others, modalHandleClose}) =>{
    const {modal,dataBrand, processorType, reloadForDB, dataStore} = others
    ...
    let  {name, id,  index, store, brand, type, link, item_active, dol, sol } = form
                ...
                {!modal &&
                    <h2 className="accordion-header">
                        <button className="accordion-button" type="button" data-bs-toggle="collapse" data-bs-target={`#panelsStayOpen-collapseOne_${nameDiv}_${id}`} aria-expanded="false" >
                            <b className={"me-3"}>Motherboard {index !== 0 && `${(index+1)}`}: </b> {name} <TextActive data={{state:item_active, center:false}}/>
                        </button>
                    </h2>
                }
                    ...
                    <Row className="accordion-body">
                        <Col md={11}>
                            <Row className={"pe-2 ps-2 pt-2"}>
                                ...
                                <InputSelect col={3} name={"type"} title={"type"} update={onChange} select={type}  data={processorType}/>
